...
fn get_dims<S: CodeSpecification>(
    n: usize,
    log2p: f64,
...
pub fn gen_matrix<F, R>(n: usize, m: usize, d: usize, mut rng: R) -> CsMat<F>
where
    F: PrimeField,
    R: Rng,
{
...
where
    ...
    S: CodeSpecification,
{
    let (pre_dims, post_dims) = get_dims::<S>(n, (F::NUM_BITS - 1) as f64);
    ...
    pre_dims[..]
        ...
        .map(|(i, (&(ni, mi, cn), &(nip, mip, dn)))| {
            ...
            let precode = gen_matrix(ni, mi, cn, &mut rng);
            let postcode = gen_matrix(nip, mip, dn, &mut rng);
            (precode, postcode)
        })
