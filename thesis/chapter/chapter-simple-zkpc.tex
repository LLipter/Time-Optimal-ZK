...
We prove it by contrapositive. Suppose there exists $l+1$ columns of generator matrix $G$ are NOT linearly independent of each other. Without loss of generality, we use $G[I_1], \cdots, G[I_l]$ and $G[I_{l+1}]$ to denote these $l+1$ columns. Then it must be possible to write $G[I_{l+1}]$ as a linear combination of $G[I_1], \cdots, G[I_l]$. Namely, 
$$
...
\end{bmatrix}  
$$
In $H$, only $c[I_{l+1}]$ is unknown. And we refer the last row as the challenge row. Then we run the Gaussian elimination algorithm on it and get the $H^\prime$ matrix in reduced row echelon form. Because these $l+1$ columns are linearly independent of each other, each row in $H^\prime$ matrix will have a pivot element. We use $j$ to denote the pivot position of the challenge row. And the challenge row should have the following form,
$$
...
$m_a$ is the $a$-th element in the message $m$. $s$ is a known value and is introduced by those row operations in the Gaussian elimination algorithm.
