...
We prove it by contrapositive. Suppose there exists $l+1$ columns of generator matrix $G$ are NOT linearly independent of each other. We use $G[I_1], \cdots, G[I_l]$ and $G[I_{l+1}]$ to denote these $l+1$ columns. Without loss of generality, it must be possible to write $G[I_{l+1}]$ as a linear combination of $G[I_1], \cdots, G[I_l]$. Namely, 
$$
